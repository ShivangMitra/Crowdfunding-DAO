...
function Cards({ setPageState, data, setCampaignIndex, weiToEther }) {
      ...
      <CardBody>
        ...
        <Stack mt='6' spacing='3'>
          ...
          <Text color='tomato' fontSize='2xl' style={{display: 'flex', justifyContent: 'space-between'}} >
            {weiToEther(data.balance)} ETH {`($${weiToEther(data.balance*1660)})`}
            <Button onClick={handleCampaignPage} variant='solid' colorScheme='orange' backgroundColor={'tomato'} rightIcon={<FaHandHoldingUsd/>}>
            Support
            </Button>
          </Text>
          <Text color={'#0000008a'} >Target of {weiToEther(data.targetAmt)} ETH {`($${weiToEther(data.targetAmt * 1660)})`}</Text>
